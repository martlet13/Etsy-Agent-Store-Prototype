...
function productTerms(productType = "") {
  ...
  if (raw === "shirt" || raw === "tshirt" || raw === "t-shirt") {
    terms.add("shirt");
    terms.add("t-shirt");
    terms.add("tee");
  }
  if (raw === "sticker" || raw === "stickers") {
    terms.add("sticker");
  }
  if (raw === "mug") {
    terms.add("mug");
...
function computeProfit(option, targetPrice) {
...
function dedupeOptions(options) {
  ...
  for (const option of options || []) {
    ...
    if (!existing || (option.score || 0) > (existing.score || 0)) map.set(key, option);
  }
  return [...map.values()];
}

function upsertSavedCostRows(rows) {
  ...
  for (const option of rows) {
    ...
    const row = normalizeSupplierCostRow({
      ...option,
      existing,
      verifiedAt: now
    });
...
export function normalizeSupplierCostRow(raw) {
  ...
  return {
    ...
    production_cost: Number(money(raw.baseCost ?? raw.production_cost ?? 0)?.toFixed(2) || 0),
    shipping_cost_us: raw.shippingCost === undefined || raw.shippingCost === null ? raw.existing?.shipping_cost_us ?? null : Number(money(raw.shippingCost)?.toFixed(2) || 0),
    currency: "USD",
    ...
    raw_api_excerpt: raw.raw ? {
      supplier: raw.supplier,
      productId: raw.productId,
      variantId: raw.variantId
    } : raw.existing?.raw_api_excerpt,
    verified_at: verifiedAt,
    updated_at: verifiedAt
  };
}

export function scoreSupplierOption(option, designPackage) {
  let score = 0;
  ...
  if (Number.isFinite(option.estimatedMarginPct)) score += Math.max(-10, Math.min(20, option.estimatedMarginPct / 2));
...
function extractPrintifyShipping(shippingJson) {
  const profiles = Array.isArray(shippingJson?.profiles) ? shippingJson.profiles : [];
  const rows = profiles.flatMap((profile) => Array.isArray(profile?.countries) ? profile.countries : []);
  const us = rows.find((row) => normalizeKey(row.country) === "united states" || normalizeKey(row.country_code) === "us") || rows[0];
...
export async function researchPrintifyProducts(input) {
  if (!process.env.PRINTIFY_API_KEY) {
    return { ok: false, designPackageId: input.designPackageId, options: [], savedCostRows: [], missingConnector: "PRINTIFY_API_KEY", error: "Printify API key is missing." };
  }
  ...
  const blueprints = (Array.isArray(blueprintsJson) ? blueprintsJson : blueprintsJson.data || [])
    .filter((item) => textMatchesProduct(`${item.title || ""} ${item.brand || ""} ${item.model || ""}`, input.productType || "poster"))
    .slice(0, 4);
  const options = [];
  ...
  for (const blueprint of blueprints) {
    ...
    for (const provider of providers) {
      ...
      const variants = (Array.isArray(variantsJson) ? variantsJson : variantsJson.variants || variantsJson.data || [])
        .filter((variant) => {
          const enabled = variant.is_enabled !== false;
          const title = `${variant.title || ""} ${variant.options?.title || ""}`;
          return enabled && (!input.productType || textMatchesProduct(`${blueprint.title} ${title}`, input.productType));
        })
        .slice(0, 4);
...
function printfulProductsArray(json) {
  if (Array.isArray(json?.result)) return json.result;
  if (Array.isArray(json)) return json;
  return [];
}

function printfulVariantsArray(json) {
  if (Array.isArray(json?.result?.variants)) return json.result.variants;
  if (Array.isArray(json?.result?.sync_variants)) return json.result.sync_variants;
  if (Array.isArray(json?.result)) return json.result;
  return [];
}
...
async function printfulShippingFor(variantId, input) {
  ...
  const first = rates[0];
  const rate = money(first?.rate);
  return { shippingCost: rate, verified: rate !== null, warning: rate === null ? "printful_shipping_rate_missing" : "" };
}

export async function researchPrintfulProducts(input) {
  if (!process.env.PRINTFUL_API_KEY) {
    return { ok: false, designPackageId: input.designPackageId, options: [], savedCostRows: [], missingConnector: "PRINTFUL_API_KEY", error: "Printful API key is missing." };
  }

  const maxOptions = Math.max(1, Number(input.maxOptions || 6));
  const productsJson = await fetchJson(`${PRINTFUL_API_BASE}/products`, { headers: printfulHeaders() });
  const products = printfulProductsArray(productsJson)
    .filter((item) => textMatchesProduct(`${item.title || ""} ${item.type || ""} ${item.name || ""}`, input.productType || "poster"))
    .slice(0, 5);
  const options = [];
  ...
  for (const product of products) {
    ...
    const variants = printfulVariantsArray(detail).slice(0, 6);
    for (const variant of variants) {
      const baseCost = money(variant.price ?? variant.retail_price ?? variant.cost);
      if (baseCost === null) continue;
      const shipping = await printfulShippingFor(variant.id, input);
      options.push({
        supplier: "printful",
...
export async function researchSupplierProducts(input) {
  const designPackages = readJson("design_packages.json", []);
  const designPackage = designPackages.find((item) => item.id === input.designPackageId);
  const suppliers = input.preferredSupplier === "printify"
    ? ["printify"]
    : input.preferredSupplier === "printful"
      ? ["printful"]
      : ["printify", "printful"];
  const allOptions = [];
  let firstError = "";
  ...
  for (const supplier of suppliers) {
    try {
      const result = supplier === "printify"
        ? await researchPrintifyProducts(input)
        : await researchPrintfulProducts(input);
